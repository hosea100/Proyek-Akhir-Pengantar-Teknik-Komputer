...
//Format Struktur File
{
 ...
 char name[50];
 int std;
 int mpkt,alin,fismek,ptk,agama,praktik;
 float rata;
}st;
...
void write_student()
   {
    ...
    gets(st.name);
    printf("\nMasukkan Nilai Mata Kuliah MPKT-A 			: ");
    scanf("%d",&st.mpkt);
    printf("\nMasukkan Nilai Mata Kuliah Aljabar Linear 		: ");
    scanf("%d",&st.alin);
    printf("\nMasukkan Nilai Mata Kuliah Fisika Mekanika 		: ");
    scanf("%d",&st.fismek);
    printf("\nMasukkan Nilai Mata Kuliah Pengantar Teknik Komputer 	: ");
    scanf("%d",&st.ptk);
    printf("\nMasukkan Nilai Mata Kuliah Agama 			: ");
    scanf("%d",&st.agama);
    printf("\nMasukkan Nilai Mata Kuliah Praktikum Fisika Mekanika 	: ");
    scanf("%d",&st.praktik);
    st.rata=(st.mpkt+st.alin+st.fismek+st.ptk+st.agama+st.praktik)/6.0;
...
void display_all()
{
    ...
    while((fread(&st,sizeof(st),1,fptr))>0)
    {
      ...
      printf("\nNilai Mata Kuliah MPKT-A 				: %d",st.mpkt);
      printf("\nNilai Mata Kuliah Aljabar Linear  			: %d",st.alin);
      printf("\nNilai Mata Kuliah Fisika Mekanika 			: %d",st.fismek);
      printf("\nNilai Mata Kuliah Pengantar Teknik Komputer 		: %d",st.ptk);
      printf("\nNilai Mata Kuliah Agama 				: %d",st.agama);
      printf("\nNilai Mata Kuliah Praktikum Fisika Mekanika 		: %d",st.praktik);
...
//****************************************************************
    ...
    void class_result()
    {
     ...
     fptr=fopen("student.dat","rb");
      ...
      printf("=====================================================================================================================================================\n");
      printf("NPM.             		Nama         			MPKT-A    ALIN    FISMEK    PTK     AGAMA   PRAKTIKUM     Rata-rata    Nilai Akhir |\n");
      printf("=====================================================================================================================================================\n");

      while((fread(&st,sizeof(st),1,fptr))>0)
     { 
       printf("%-6d 		%-10s 			%-3d 	  %-3d 	    %-3d      %-3d 	 %-3d        %-3d 	    %-3.2f  	    %-1c\n",st.npm,st.name,st.mpkt,st.alin,st.fismek,st.ptk,st.agama,st.praktik,st.rata,st.grade);
     }
     printf("\n\n\n\n\n\nTekan ENTER untuk kembali ke menu utama\n");
     fclose(fptr);
     getch();
...
void entry_menu()
{
  ...
  switch(ch2)
  {
    ...
    case '3':  {
      ...
      case '6': break; //Kembali ke menu utama
      default:	printf("\a");
	  			entry_menu();
   }
}
...
//sumber referensi		 : https://www.youtube.com/watch?v=TU9Hv8ThArk									=> Mengambil Tampilan bagan
//						   http://mini-projetcs-in-c.blogspot.co.id/2011/04/student-report-card.html	=> Mengambil Struktur Data Program
